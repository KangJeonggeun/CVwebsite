...
// const animColor = "#FCD34D"; // original animcolor "#FCD34D"
...
const TimelineSection = ({ isDesktop }: IDesktop) => {
  ...
  // console.log({ svgCheckpointItems, i18n });
  ...
  const addText = (
    ...
    const titleKey = String(title) + "." + "title";
    const subtitlekey = String(title) + "." + "subtitle";
    const subtitleString1 = subtitle ? t(subtitlekey) : "";

    return `<foreignObject x=${foreignObjectX} y=${foreignObjectY} width=${foreignObjectWidth} 
        height=${separation}>${logoString}<p class='${titleSizeClass}'>${t(
      titleKey
    )}</p>${subtitleString1}</foreignObject>`;
  };
  ...
  const renderSlides = (): React.ReactNode => {
    // const a = i18n.language;
    // console.log("renderslides", { a });
    return (
      <div
  ...
  const renderSectionTitle = (): React.ReactNode => {
    return (
      <div className="flex flex-col">
        <p className="section-title-sm seq">{t("timeline.MILESTONES")}</p>
        <h1 className="section-heading seq mt-2">{t("timeline.Timeline")}</h1>
        <h2 className="text-2xl md:max-w-2xl w-full seq mt-2">
          {t("timeline.quickRecap")}
        </h2>
      </div>
    );
  };
